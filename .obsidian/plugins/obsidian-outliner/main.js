...
class DragAndDropState {
  ...
  calculateTabWidth() {
    ...
    const singleIndent = language.indentString(
      view.state,
      language.getIndentUnit(view.state),
    );
    ...
    this.tabWidth =
      view.defaultCharacterWidth * language.getIndentUnit(view.state);
  }
}
      ...
      if (e.is(dndMoved)) {
        dndDroppingState =
          e.value === null
            ...
            : view.Decoration.set(
                droppingLineDecoration.range(e.value, e.value),
              );
      }
      if (e.is(dndEnded)) {
...
class EnterBehaviourOverride {
  constructor(
    ...
    this.check = () => {
      return (
        this.settings.overrideEnterBehaviour && !this.imeDetector.isOpened()
      );
    };
    this.run = (editor) => {
      const root = this.parser.parse(editor);
  ...
  constructor(plugin, settings, obsidianSettings, parser) {
    ...
    this.updateBodyClass = () => {
      const shouldExists =
        this.obsidianSettings.isDefaultThemeEnabled() &&
        this.settings.verticalLines;
      const exists = document.body.classList.contains(
